...
Results are written to comparison/results/protocol_conformance.json so the
report and its figures regenerate from measured data with no re-run.

    python -m comparison.exp_protocol
"""
...
# The message sequence src/orchestrator.py emits for one trip, in order. Kept
...
def audit_a2a() -> List[Dict[str, Any]]:
    from src.comms.protocol import (A2AMessage, A2AProtocol, MessagePriority,
                                    MessageQueue, MessageType)
    from src.comms.registry import AGENT_REGISTRY, validate_communication
...
def audit_mcp() -> List[Dict[str, Any]]:
    import src.server.mcp_server as server
