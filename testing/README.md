...
```bash
python -m pytest -q        # 301 tests, from the project root
```
...
| `test_real_price_validation.py` | 22 | That the budget check can use a fare the API really quoted instead of a constant, says which it used, and that the DEFAULT stays the table — the published Cohen's kappa depends on it. |
