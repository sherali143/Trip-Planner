...
```bash
python -m pytest -q        # 385 tests, from the project root
```
...
| `test_real_price_validation.py` | 29 | That the budget check can use a fare the API really quoted instead of a constant, says which it used, and that the DEFAULT stays the table — the published Cohen's kappa depends on it. |
...
| `test_run_narration.py` | 13 | That the line each step prints says what happened. A failed hotel search returns an explanation as an ordinary string, and was being reported as "returned 371 chars" — a step that looked like it worked. |
