...
def build(report: Report) -> None:
    ...
    report.p("""
        ...
        costs per trip and how grounded its output is. E2 and E3 re-read E1's records
        to ask where the token spend goes and how much of the multi-agent penalty is
    ...
    report.p(f"""
        ...
        other trips cannot, and this remains a single-case study in the sense used in
        empirical software engineering [@runeson2009; @wohlin2012].
    """)
    ...
    report.p(f"""
        ...
        The tool-less arm issues exactly
        {val(measured.spread('A', 'llm_calls')['mean'], '{:.0f}')} request and the
        direct arm exactly {val(measured.spread('D', 'llm_calls')['mean'], '{:.0f}')}
        on every run, because neither contains a loop deciding how many requests to
        make. The agent arms cannot offer that — the naive arm varies by
    ...
    report.p(f"""
        ...
        [{val(cost_c['ci95_low'], '${:.4f}')}, {val(cost_c['ci95_high'], '${:.4f}')}]
        and the direct arm in
        [{val(cost_d['ci95_low'], '${:.4f}')}, {val(cost_d['ci95_high'], '${:.4f}')}].
        Latency intervals do not overlap either —
        [{val(lat_c['ci95_low'], '{:.1f}')}, {val(lat_c['ci95_high'], '{:.1f}')}]
        seconds against
        [{val(lat_d['ci95_low'], '{:.1f}')}, {val(lat_d['ci95_high'], '{:.1f}')}].
        Both differences are therefore larger than run-to-run variation.
    """)

    report.p(f"""
        Groundedness behaves the other way, and this is the result the chapter turns
        on. The tuned arm's interval is
        [{val(gnd_c['ci95_low'], '{:.1f}')}, {val(gnd_c['ci95_high'], '{:.1f}')}]%
        and the direct arm's is
        [{val(gnd_d['ci95_low'], '{:.1f}')}, {val(gnd_d['ci95_high'], '{:.1f}')}]%.
        They overlap substantially. The tuned arm's mean is higher —
        {val(gnd_c['mean'], '{:.1f}')}% against {val(gnd_d['mean'], '{:.1f}')}% — and
        with five observations apiece that difference cannot be distinguished from
        noise. The honest reading is not that the two are equal but that this
    ...
    report.p("""
        ...
        count, makes request cost exactly predictable, and costs no groundedness this
    ...
    report.p(f"""
        ...
        against the direct arm's
        {val(measured.arm_metric('D', 'avg_cost_usd'), '${:.4f}')} despite issuing one
        request rather than two, and it was the slowest arm at
        {val(measured.arm_metric('A', 'avg_latency'), '{:.0f}')} seconds. The next
    ...
    report.p(f"""
        ...
        {val(a_llm['completion_tokens'], '{:,.0f}')} completion: with no data to work
        from it invented detail to fill the requested structure. Output is billed at
        several times the rate of input, so an arm that reads nothing and writes a
        great deal is expensive, and its latency follows for the same reason.
    """)
    ...
    report.p(f"""
        ...
        {val(b_llm['prompt_tokens'] / (b_llm['prompt_tokens'] + b_llm['completion_tokens']) * 100, '{:.0f}')}%
        of its spend is re-sent context. Raw tool output for the whole scenario is a
        small fraction of that, so the payload is not the problem — re-sending the
        accumulated transcript plus every bound tool's schema on every reasoning
        iteration is. This is the context-bloat mechanism from Section 2.1, measured
        [@schick2023; @liu2024].
    """)
    ...
    report.p(f"""
        ...
        implementation quality, not architecture. Run against the naive arm alone the
        comparison would have reported a {val(d_vs_b['tokens_pct'], '{:.1f}')}% token
        advantage, and the conclusion would have been unattributable: a reader could
        not tell how much came from removing agents and how much from configuring them
        competently. The defensible claim is narrower. Against a competently configured
        multi-agent baseline, removing the model from deterministic retrieval saves
        request count and latency decisively and cost only modestly, at no measured
        loss of groundedness. That claim survives the obvious objection; the wider one
        does not.
    """)
    ...
    report.p(f"""
        ...
        proposal committed to a schema pass rate of 100% and an inter-agent error rate
        below 1%; the second is met on the path that ships and the first is not met at
        all. The six failures are findings about this artefact, not incidental defects.
    """)

    report.p(f"""
        Three failures are in the message layer. Priority is declared on every message
        and never honoured, because the queue is first-in-first-out: enqueued
        {a2['observed']['enqueued']}, delivered
        ...
        {val(a1['observed']['declared_outbound_edges'])} declared edges asymmetric. And
        a message polled by the wrong recipient is destroyed, because the receive path
        removes it from the queue before checking the address.
    """)
    ...
    report.p(f"""
        ...
        {val(schema['undeclared_parameter_count'])} in total — so an agent cannot set
        them and they silently take defaults, which is precisely the class of mismatch
        the schema layer was adopted to prevent. One tool treats a parameter as
        mandatory that its schema does not list as required. And one advertises the
        flight scraper in its description while its dispatcher calls the hotel
        provider's flight endpoint, so an agent choosing it on that description gets a
        different backend. Appendix B gives every check and its observation.
    """)

    report.p("""
        Two general readings follow. Adopting a protocol is not conforming to it: the
        specification governs how a schema is declared and transported and leaves its
        correspondence with the code behind it to the implementer
        [@anthropic2024; @hou2025], and nothing warned. And the message-layer defects
        sit in code the shipped path never executes, which is how they survived a
        passing suite. All six were found in an afternoon by an experiment needing no
        network, no model and no credentials, after weeks in which none was noticed.
    """)
    ...
    report.p(f"""
        Two of the twenty scenarios were written to be unaffordable. The gate refused
        {val(agreement['true_positive'])} of them and accepted
    ...
    report.p(f"""
        ...
        gate estimated a minimum of
        {val(miss['estimate_minimum'], '${:,.0f}')}, judged the budget
        {val(miss['budget_vs_minimum'], '{:.2f}')} times that floor, returned a
        verdict of "{miss['verdict'].replace('_', ' ')}" and proceeded.
    """)
    ...
    report.p(f"""
        ...
        fare the API actually returned was
        {val(anchor['cheapest_real_fare'], '${:,.0f}')} and the median was
        {val(anchor['median_real_fare'], '${:,.0f}')}. The anchor is
    ...
    report.p(f"""
        ...
        {val(len(monotonic['violations']))} violations — so the structure is right and
    ...
    report.p(f"""
        Following one scenario through shows how the measures interact. The request is
        a four-night trip from Lahore to Istanbul for one traveller on eight hundred
        dollars, with an interest in history, food and shopping. Extraction produces
        structured fields. The gate estimates a minimum of
        {val(gate_row['estimate_minimum'], '${:,.0f}')} and a comfortable figure of
        {val(gate_row['estimate_comfortable'], '${:,.0f}')}, returns a verdict of
        workable, warns and proceeds. Retrieval returns
        {val(len(truth['hotels']))} hotels, {val(len(truth['airlines']))} airlines and
    ...
    report.p(f"""
        ...
        many — and matched {val(control['prices_grounded'])}. It named a real airline
        for the route despite calling no API, which is why entity matching is
    ...
    report.p(f"""
        This is the clearest result in the project and the least surprising: the
        cheapest way to produce an itinerary is to invent it, and the difference
        between a plausible plan and a usable one is the tool layer.
    """)
    ...
    report.p(f"""
        ...
        conformance audit compares declarations against implementations, and the gate
        evaluation compares decisions against intent recorded before any measurement.
        Neither outcome was chosen and both went against the design. A study that
        produced {val(measured.protocol_summary()['failed'])} failures against its
        own author is harder to dismiss than one that confirmed everything.
    """)
