...
def build(report: Report) -> None:
    ...
    report.p("""
        ...
        formal treatment would have avoided the quota problem and produced no
        evidence that any of it works against a live endpoint, which is where every
        interesting failure in Chapter 5 occurred.
    """)
    ...
    report.p("""
        Cycle one built the artefact the proposal specified and ended when the
        system was found producing confident itineraries that cited nothing
        retrievable, because a path defect meant every tool call was failing
        silently. Cycle two added instrumentation, and the resulting count motivated
        the three-agent design. Cycle three removed the obvious objection to that
        comparison by tuning the six-agent arm first, and produced the result that
        weakened the project's own thesis.
    """)
    ...
    report.p("""
        The comparison could have been run as three agents against six, and that
        design is unsound in two directions at once. Against the six-agent
        implementation as first written, any advantage the three-agent design showed
        is open to the objection that the baseline was badly configured. Without a
        tool-less control, nothing establishes that the tool layer contributes at
        all, and the cheapest architecture appears to win while its output is
        unverifiable.
    """)
    ...
    report.p("""
        ...
        ordinary Python, which is the claim under test. Appendix J tabulates the
        role each plays.
    """)
    ...
    report.p("""
        ...
        version the strongest possible position before comparing against it. It also
        produced the result in Section 6.3: that most of the difference the project
        set out to demonstrate was implementation quality rather than architecture.
        Without Arm C that finding is unavailable and the headline claim is
    ...
    report.p(f"""
        ...
        than to sample uniformly. Those axes are trip length, from three to fourteen
        nights; distance, from short-haul Dubai to long-haul Tokyo; party size, from
        solo to a family of four; destination price tier; multi-city routes; and
        budgets from generous down to deliberately impossible. Every arm receives
        the identical request string, so nothing in the comparison depends on
        phrasing.
    """)
    ...
    report.p("""
        ...
        literature describes [@xie2024], and only an impossible case exposes it.
        Those two made the budget-gate result in Section 6.5 possible, and they also
        caught a defect in the gate itself. Each scenario further records the facts
        a reader extracts from its request — origin, legs, nights, date, budget,
        party — as declared ground truth. No arm reads that field; it exists so the
        gate can be evaluated with exact parameters rather than parsed guesses.
    """)
    ...
    report.p("""
        ...
        equals would overstate the result. Matching a name is weak evidence: on the
        recorded scenario the arm with no tool access named a real airline for the
        route, having called nothing. Matching a price is strong, because landing
        within 2% of a fare quoted minutes earlier is not something prior knowledge
        delivers. The tolerance exists because an itinerary legitimately rounds and
        sums nightly rates into totals, and it is narrow enough that invention does
        not pass it. Price grounding carries the claim.
    """)
    ...
    report.p(f"""
        ...
        instrument, so a hand-counted figure can never be compared against a
        measured one. The arm with deterministic retrieval runs first in each
        scenario, which populates the response cache with canonical parameters so
        the agent arms replay rather than issuing slightly different live queries.
        Results are checkpointed after each scenario, so an interrupted run keeps
        what it has already paid for. Section 5.5 describes the mechanisms.
    """)
    ...
    report.p(f"""
        ...
        search costs two because the endpoint is asynchronous and must be polled.
        One scenario therefore consumes roughly four of eighty monthly requests, and
        ...
        recorded, which is the real sample size for the cost comparison and is
        quoted as such wherever those numbers appear.
    """)

    report.p("""
        This shaped what could be claimed. The response was not to present one
        scenario as twenty, but to answer completely those questions needing no
        quota: protocol conformance needs no network, and the feasibility gate needs
        neither network nor model. Both are evaluated across the full scenario set,
        and both produced negative findings that the quota-limited comparison could
        not have produced. Quota was also this project's largest risk: Appendix M
        holds the register, the controls built before the arms were, and which risks
        occurred regardless.
    """)
    ...
    report.p("""
        ...
        first check-in. All API use is read-only and within published free-tier
        terms, and no endpoint was scraped. The response cache holds third-party
        commercial data — hotel names, review scores, fares — solely to reproduce
        the reported results. It holds no personal data, and request headers
        carrying credentials are excluded from what is written to disk.
    """)
