...
def build() -> str:
    ...
    doc.p("""
        Two folders: all the code in one, everything handed in in the other.
        Nothing else at the top level except the ways to start it.
    """)
    doc.code(
        ...
        "README.md                what it is, and this layout\n"
        "requirements.txt         the exact package versions used\n"
        ".env.example             the four API keys, as a template\n"
        ".env                     your real keys (never shared, never committed)\n"
        "\n"
        ...
        "  core\\                 budgets, costs, caching, measuring, safe maths\n"
        "  frontend\\             the web page, and the code that splits a\n"
        "                         finished plan into its sections\n"
        "  comms\\                the message protocol between components\n"
        "  server\\               the tool server (12 tools)\n"
        "  tools\\                anything that calls an outside service\n"
        "\n"
        ...
        "    exp_protocol.py        checks the protocols against the design\n"
        ...
        "\n"
        f"  tests\\                the test suite ({_test_count()} tests), plus two\n"
        ...
        "  build\\                          the scripts that generate the first\n"
        f"                                   three, and all {_figure_count()} figures\n"
        "\n"
