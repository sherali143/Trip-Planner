...
WHAT THIS FILE DOES
===================
The web interface. Collects the traveller's details through a fixed sequence of
questions, then hands the transcript to the orchestrator and displays the
itinerary it returns.

The questions are a fixed list rather than a conversation with the
conversational agent, because Streamlit re-runs this entire script on every
interaction, which does not suit a streaming multi-turn dialogue. The
conversational agent remains in the command-line path (`run_cli.py`). This is a
deviation from the proposal and is recorded as one in the dissertation.

Everything below the form is approach D: extraction, then retrieval in plain
Python, then assembly.
"""
