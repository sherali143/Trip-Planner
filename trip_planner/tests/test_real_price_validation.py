...
class TestTheDefaultIsUntouched:
    ...
    def test_the_experiment_does_not_pass_a_probe(self):
        ...
        source = open(os.path.join(root, "trip_planner", "evaluation", "exp_budget_gate.py"),
                      encoding="utf-8").read()
...
# meant depended on the working directory. Starting anything from inside trip_planner/demos/
# created trip_planner/demos/.api_cache and used that: a replay that found nothing and looked
# broken, or a recording that spent real quota into a directory nothing else
# reads. An empty trip_planner/demos/.api_cache left behind is what gave it away.
...
def test_the_cache_directory_does_not_depend_on_the_working_directory():
    """
    A demo run from inside trip_planner/demos/ must read the committed recordings, not create
    ...
    here = os.getcwd()
    try:
        os.chdir(os.path.join(root, "trip_planner", "demos"))
        from_subdir = os.path.realpath(_cache_dir())
    finally:
        os.chdir(here)

    assert from_subdir == os.path.realpath(os.path.join(root, ".api_cache"))
    assert not os.path.isdir(os.path.join(root, "trip_planner", "demos", ".api_cache")), (
        "resolving the cache created trip_planner/demos/.api_cache again")
