...
```bash
python -m pytest -q        # 403 tests, from the project root
```
...
| `test_web_interface.py` | 71 | That the page renders, asks how many people are travelling, and refuses a form it cannot plan. Nothing tested the page at all, which is how it came to collect eight facts without ever asking the traveller count — a number the feasibility check multiplies by. |
...
`trip_planner/tests/manual/`. Between them they contributed six "passing" tests and zero
