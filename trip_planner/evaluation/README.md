# `trip_planner/evaluation/` — the experiment
...
TRIP_PLANNER_API_MODE=replay python -m trip_planner.evaluation.run_comparison

# one or more specific scenarios
TRIP_PLANNER_API_MODE=replay python -m trip_planner.evaluation.run_comparison SC-01 SC-04

# a live batch, bounded in both directions
TRIP_PLANNER_MAX_LLM_CALLS=180 TRIP_PLANNER_MAX_LIVE_CALLS=18 \
TRIP_PLANNER_API_MODE=record python -m trip_planner.evaluation.run_comparison
```
...
python -m trip_planner.evaluation.exp_budget_gate   # budget feasibility gate, 20 scenarios
```
