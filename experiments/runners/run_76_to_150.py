...
def main():
    print("=" * 60)
    print("RUNNING 75 MORE ITERATIONS (Runs 76-150)")
    ...
    for i in range(75):
        ...
        if run_result.get("success"):
            ...
            if run_result.get("feasible"):
                feasible += 1
            print(f"✅ Run {run_id}: Tokens={run_result.get('tokens', 0):,} | Latency={run_result.get('latency', 0):.1f}s | Feasible={run_result.get('feasible')}")
        else:
            print(f"❌ Run {run_id}: FAILED - {run_result.get('error', 'Unknown')[:50]}")
        
        # Save incrementally
        with open("experiment_runs_76_150.json", "w", encoding='utf-8') as f:
    ...
    print("\n" + "=" * 60)
    print("📊 BATCH 2 COMPLETE (Runs 76-150)")
    ...
    print(f"Results: experiment_runs_76_150.json")
    print("=" * 60)
